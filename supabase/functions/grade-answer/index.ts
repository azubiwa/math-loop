...
export default {
  fetch: withSupabase({ auth: "user" }, async (request, context) => {
    ...
    if (!apiKey)
      return json({ error: "Sakura AI Engine is not configured" }, 503);
    ...
    try {
      ...
      const prompt =
        ...
        body.prompt.every((item) => typeof item === "string")
          ? body.prompt
          : null;

      if (
        !/^(?:(?:MABC\d{3}|L[1-3]-\d{3})|GSET-\d{3})-[A-E]$/.test(problemId) ||
        !title ||
        !answer ||
        !prompt
      ) {
      ...
      if (hourlyUsage.error || dailyUsage.error || monthlyUsage.error) {
        console.error(
          "Unable to check grading quota",
          hourlyUsage.error || dailyUsage.error || monthlyUsage.error,
        );
        return json({ error: "Grading is temporarily unavailable" }, 503);
      }
      if (
        (hourlyUsage.count || 0) >= 60 ||
        (dailyUsage.count || 0) >= 100 ||
        (monthlyUsage.count || 0) >= 2_900
      ) {
        return json(
          { error: "Grading limit reached. Please try again later." },
          429,
          { "retry-after": "3600" },
        );
      }
      ...
      if (!response.ok) {
        ...
        return json(
          { error: `Sakura AI Engine returned ${response.status}` },
          status,
        );
      }
