...
メニューの「25マス計算」から、縦横それぞれ5つの対象を組み合わせる計算表に取り組めます。5分野・21種類、全380セット（9,500マス）を収録しています。

- 計算：整数加算・乗算、分数加算、複素数乗算（各10セット）
- 代数：ベクトルの内積、行列積
- 解析：多項式の微分、常微分方程式、テイラー展開、複素解析
- 構造：最大公約数、剰余
- 幾何：1形式の評価、wedge積、df、外微分、pullback、リーブラケット、計量と内積、クリストッフェル記号、接ベクトルと微分

追加17種類は基礎・標準を各10セット収録。同じセット番号では同じ問題を出題します。練習では1マスずつ確認し、係数入力・数式プレビュー・解説・誤答復習を利用できます。計測では開始後に問題を表示し、提出後に正誤を表示します。多項式は専用パーサーで同値性を厳密に採点します。

各項目には「基本の解き方と計算例」を用意し、公式・手順・具体例・注意点を確認できます。計測中はガイドを閉じます。常微分方程式は初期条件つき多項式解、複素解析は正則多項式の導関数の値を扱う入門ドリルです。
